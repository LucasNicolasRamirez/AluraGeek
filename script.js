...
document.getElementById('add-product-form').addEventListener('submit', async function (event) {
    ...
    try {
        ...
        const response = await fetch('https://675fc6d91f7ad2426999495f.mockapi.io/api/v1/productos', {
            method: 'POST',
...
function renderProduct(product) {
    ...
    newProduct.querySelector('.eliminar').addEventListener('click', async function () {
        try {
            const response = await fetch(`https://675fc6d91f7ad2426999495f.mockapi.io/api/v1/productos/${product.id}`, { method: 'DELETE' });
            if (response.ok) {
